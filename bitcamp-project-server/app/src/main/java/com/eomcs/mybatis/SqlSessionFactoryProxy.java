...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  //    해당 스레드는 스레드 보관소에 저장된 SqlSession 객체를 제거해야한다.
  // => 그래야 다음 작업을 처리할 때 초기 상태에서 작업을 수행할 수 있다.
  public void closeSession() {

    // 1) 스레드 보관소에 들어있는 SqlSession 객체를 꺼낸다.
    SqlSessionProxy sqlSessionProxy = threadLocal.get();
    if (sqlSessionProxy != null) {
      // 사용한 SqlSession 객체를 완전히 닫는다.
      // => 자원은 항상 쓰고 닫아야 한다.
      // => 특히 24시간 365일 내내 실행하는 서버인 경우 자원 해제는 메모리 관리차원에서 아주 중요하다.
  ...
  public SqlSession openSession(boolean autoCommit) {
    ...
    // 트랜젝션 관리자의 통제 없이 독자적으로 수동 커밋으로 동작하는 SqlSession이 필요하다는 뜻이기 때문에
